...
/*   Updated: 2025/02/24 13:53:46 by nafarid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void check_is_rec(t_map *map)
{
    int x;
    int y;
    int height;
...
static void check_is_closed(t_map *map)
{
    int i;
...
void check_elements(t_map *map)
{
    ...
    {
        var->x = 0;
        while(map->map[var->y][var->x] == 'E')
            var->e++;
        while(map->map[var->y][var->x] == 'P')
            var->p++;
        while(map->map[var->y][var->x] == 'C')
            var->c++;
        while(map->map[var->y][var->x] != '1'
            || map->map[var->y][var->x] != '0')
            filed_elements(var);
        var->y--;
    }
